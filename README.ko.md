...
1. 방의 가로·세로를 cm로 입력하고 **이 크기로 시작**을 누릅니다.
2. 가구를 검색해 고른 뒤 도면의 원하는 곳을 누릅니다. 본체를 끌어 옮기고, 선택 막대의 **크기**·**회전**으로 조절합니다. 실행 취소 후에도 남아 있는 가구의 선택은 유지됩니다.
3. **3D 미리보기**에서 배치를 확인합니다. 천장·발표용 벽·선택 초점·PNG 저장은 **도구 더보기**에 있습니다.

기본은 도면을 가리지 않는 간편 배치입니다. **정밀 도구**를 켜면 같은 도면에서 벽·치수·좌표 등 전체 기능을 사용합니다. 상담 기록과 A/B 비교는 **배치 비교 · 상담 · 제안서**에서 열 수 있습니다. 작업은 이 브라우저에 자동 저장되며, 다른 브라우저로 옮길 때는 **파일**로 보관하세요.

### 지역 아파트 참고 샘플

**시작 → 아파트 샘플 체험**에서 대치동 래미안대치팰리스를 열 수 있습니다. **샘플**에는 압구정 현대 35평형과 도곡렉슬 33A도 있습니다. 침실·욕실·발코니의 문과 열린 공용 공간을 통해 1인칭으로 둘러볼 수 있도록 구성했습니다.

공개 평면을 참고한 재구성이며 실측 도면이 아닙니다. [출처·표기 면적·추정 범위](docs/REGIONAL_SAMPLES.md)를 확인하세요.
...
아래 이미지는 이전 버전의 정밀 편집·3D 기능 참고 화면이며, 새 기본 화면인 간편 배치와는 구성이 다릅니다. 이미지를 선택하면 원본 크기로 자세히 볼 수 있습니다.
...
- 고객·업체·요구사항을 기록하고 독립적인 배치안 A/B를 비교한 뒤 고객용 제안서로 전달
- 방, 거실, 주방, 다용도실 등 공간 추가·이동·크기 조정·삭제
- 배경 도면 PNG/JPG 가져오기, 두 점의 실제 거리로 축척 보정, 투명도와 이동 잠금
- 시작점·끝점을 직접 찍는 영구 치수선과 cm 단위 벽 길이 직접 입력
...
- 선택 대상 복제·복사/붙여넣기·잠금과 방향키 1cm 미세 이동
...
- 간편 모드에서 첫 터치부터 가구 드래그, 도면을 가리지 않는 선택 막대와 필요할 때만 여는 크기 손잡이
- 정밀 모드의 작업 메뉴·길게 눌러 이동, 빈 도면 탭 선택 해제
- 모바일 그룹 선택 작업바, 두 손가락 확대·축소, 터치 크기 조절, 44px 입력·조작 영역과 3D 방향 패드
- PPT처럼 보이는 선택 경계와 44px 크기·연속 회전 핸들, 정확한 회전 각도 입력, 이동·크기·각도의 실시간 캔버스 피드백
- 자연스러운 바닥 재질, 천장 조명, 사용자가 배치한 문 개구부와 가구별 실제 형태를 반영한 1인칭 WebGL 워크스루
- 1인칭·돌하우스·상공 시점 전환, 천장 숨기기, 2D 선택 대상 바로 보기와 현재 3D 화면 PNG 저장
...
1. 샘플이나 기존 도면을 연 뒤 **배치 비교 · 상담 · 제안서**를 펼치고 **상담 정보**에서 프로젝트 이름·업체명·고객명·요구사항을 입력합니다.
2. A안을 편집하고 **B안 만들기**를 누른 뒤 B안에서 다른 배치를 만듭니다. 추천 이유와 수정 사항은 각 안에 따로 기록합니다.
3. **비교**를 누르면 데스크톱에서는 두 안을 나란히, 모바일에서는 위아래로 확인합니다. 3D 전체보기는 벽을 낮춰 가구를 보여주며 1인칭에서는 원래 벽 높이와 통행 충돌을 유지합니다.
4. **제안서**로 업체명, 두 배치안, 추천 이유, 치수·문 표현, 대상별 확인 사항이 담긴 HTML 문서를 받습니다. 별도 계정 없이 열거나 A4로 인쇄할 수 있습니다.

프로젝트 이름과 상담 메모는 새로고침·도면 파일 이동 후에도 유지됩니다. 안을 전환하면 실행 취소 이력이 초기화되어 다른 안을 바꾸지 않습니다. 새 도면이나 파일로 교체하기 전에는 복구본 하나를 남기며, 저장 실패 시 파일 보관과 다시 저장을 선택할 수 있습니다.

새 파일은 스키마 3을 사용하고 이전 스키마 1·2도 읽습니다. 두 안의 배경 이미지를 포함한 전체 파일 제한은 1 MiB입니다. 바닥 점유는 가구 외곽 크기를 기준으로 한 추정치이며 실제 통행 폭·설치 여유는 별도로 확인해야 합니다.
...
- 최초 로그인: 현재 로컬 상담을 유지하고 `지금 저장`으로 계정에 보관
- 로그인 후 편집: 1.2초 디바운스 클라우드 자동 저장
- 다른 탭·기기에서 먼저 저장한 경우: 현재 상담을 유지하고 별도 프로젝트로 복사하거나 복구본을 남긴 뒤 서버 도면 열기
