...
const optionName = (data, option) => data.options[option].label === `${option}안`
  ? `${option}안` : `${option}안 · ${data.options[option].label}`;
...
export function renderConsultationToolbar({ projectName, consultation }) {
  ...
  return `<section class="consultation-toolbar" aria-label="상담 및 배치안">
    <div class="consultation-context"><span title="${escapeHtml(`${projectName} · ${data.options[data.activeOption].label}`)}">${escapeHtml(data.clientName || '고객 정보 미입력')} · ${escapeHtml(data.options[data.activeOption].label)}</span><button type="button" data-consultation-open>상담 기록</button></div>
    <div class="consultation-option-controls">
      ...
      <button type="button" data-options-compare ${hasB ? '' : 'disabled title="B안을 만들면 비교할 수 있습니다."'}>A/B 비교</button>
      <button class="consultation-report-button" type="button" data-consultation-report>고객 제안서</button>
    </div>
  </section>`;
}
...
export function renderConsultationDialog({ projectName, consultation }) {
  ...
  return `<div class="consultation-backdrop" data-consultation-backdrop>
    <section class="consultation-dialog" role="dialog" aria-modal="true" aria-labelledby="consultation-dialog-title">
      <header class="consultation-dialog-header"><div><span class="eyebrow">프로젝트와 배치안 기록</span><h2 id="consultation-dialog-title">상담 정보 편집</h2></div><button type="button" data-consultation-close aria-label="변경 취소하고 상담 정보 닫기">닫기</button></header>
      <form data-consultation-form>
        <div class="consultation-form-scroll">
          ...
          <fieldset class="consultation-fieldset"><legend>${escapeHtml(optionName(data, data.activeOption))} 설명</legend>
...
export function renderComparisonDialog({ projectName, layout }) {
  ...
  return `<div class="consultation-backdrop" data-comparison-backdrop>
    ...
    const geometry = geometryForOption(layout, option);
    const itemCount = geometry.items.length;
    const area = (calculateUnionArea(geometry.zones) / 10000).toFixed(1);
    return `<article class="comparison-option" data-comparison-option="${option}">
          <header><h3>${escapeHtml(name)}</h3><div class="comparison-markers">${data.activeOption === option ? '<span>현재 편집 중</span>' : ''}${data.recommendedOption === option ? '<strong>추천안</strong>' : ''}<span data-comparison-items="${itemCount}">가구 ${itemCount}개</span><span data-comparison-area="${area}">공간 ${area}m²</span></div></header>
          <div class="comparison-plan">${renderPlanSvg(geometry, { label: `${projectName} · ${name} 도면`, idPrefix: `comparison-${option}` })}</div>
